"""`duckterm doctor` — one diagnostic that explains why sessions aren't showing.
...
def _check_server(url: str) -> Result:
    """A running Duckterm answers / with its self-probe header."""
    try:
        ...
        return Result(
            ...
            "start it: duckterm serve",
        )
    if resp.headers.get("X-Duckterm") == "1":
        return Result("ok", f"server reachable at {url}")
    return Result(
        "fail",
        f"{url} is answered by something that isn't Duckterm",
        "free the port or pick another: duckterm serve --port 4300",
    )


def _check_token() -> Result:
    """The hook reads ~/.duckterm/token to authenticate its POSTs."""
    path = paths.home() / "token"
    if not path.exists():
        return Result(
            "warn",
            "auth token not created yet",
            "it's written on first `duckterm serve`; start the server once",
        )
    try:
        readable = bool(path.read_text().strip())
    except OSError as e:
        return Result(
            "fail", f"token file unreadable: {e}", "check permissions on ~/.duckterm/token"
        )
...
def _check_hook_installed(agent: str) -> Result:
    ...
    if not path.exists():
        return Result(
            ...
            f"install: duckterm install-hooks --agent {agent} --global",
        )
    blob = path.read_text()
    if "duckterm" not in blob:
        return Result(
            "warn",
            f"{agent}: settings exist but no Duckterm hook",
            f"install: duckterm install-hooks --agent {agent} --global",
        )
    if script not in blob:
        return Result(
            ...
            f"reinstall: duckterm install-hooks --agent {agent} --global",
        )
    if agent == "codex" and '"async": true' in blob:
        return Result(
            "fail",
            "codex: hook has async:true — Codex skips async hooks (events never arrive)",
            "reinstall to drop it: duckterm install-hooks --agent codex --global",
        )
    if agent == "codex":
        return Result(
            "ok",
            ...
            "trust the Duckterm hook (once per machine; re-trust if the script changes)",
        )
