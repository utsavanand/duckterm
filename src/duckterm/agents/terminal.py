...
terminal you can type into. The dashboard's own terminals don't come through
here — those are PTYs the orchestrator owns, rendered in the browser.

The terminal app is chosen by:
  1. an explicit `app` argument ("iterm" | "terminal"), else
  2. the DUCKTERM_TERMINAL env var, else
...
def open_in_terminal(
    ...
    *,
    ...
    `env` is exported before the command (used to pass DUCKTERM_SESSION_KEY so
    the agent's hooks report under Duckterm's session key). `heartbeat` is
    ...
    if os.environ.get("DUCKTERM_NO_TERMINAL"):
        return False

    if system == "Darwin":
        choice = (app or os.environ.get("DUCKTERM_TERMINAL") or _default_mac()).lower()
    ...
    print(f"[duckterm] open a terminal and run:\n  {command}")
    return False
...
def with_heartbeat(agent: str, url: str, session_key: str) -> str:
    ...
    # every ping so the server can tell sessions in different terminals apart.
    #
    ...
    token_var = '__rd_tok=$(cat "${DUCKTERM_HOME:-$HOME/.duckterm}/token" 2>/dev/null); '
