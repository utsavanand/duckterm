"""Dev-only auto-reload for `duckterm serve --reload`.

Polls the mtimes of every .py under src/duckterm once a second on a daemon
...
def _watch(initial: dict[str, float]) -> None:
    while True:
        ...
        if changed is not None:
            print(
                f"[duckterm] {Path(changed).name} changed — restarting",
                file=sys.stderr,
                flush=True,
            )
