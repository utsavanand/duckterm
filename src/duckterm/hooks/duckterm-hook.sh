#!/bin/bash
# Duckterm ingest hook. Claude Code runs this on each hook event with the
# event's JSON on stdin; it forwards the event to the Duckterm server.
#
# Wired automatically by `duckterm install-hooks`. Self-contained — no
...
# slow Duckterm never wedges the agent.
case "$EVENT_TYPE" in
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
  STATUS=$(curl -s -m 5 "$URL/approvals/$RID/decision" \
    -H "X-Duckterm-Token: $TOKEN" 2>/dev/null | jq -r '.status // "gone"')
  case "$STATUS" in
    approve)
