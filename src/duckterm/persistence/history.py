"""Durable mirror of the event stream in SQLite at ~/.duckterm/db.sqlite.
...
_SESSIONS_COLUMNS = {
    ...
    "ended_at": "INTEGER",
    "heartbeat": "INTEGER NOT NULL DEFAULT 0",
    # 1 when Duckterm launched the session itself (vs a watched/hook session).
...
class HistoryStore:
    ...
    def clear_tombstones(self) -> int:
        ...
        and its session reappears. `duckterm restart` calls this — the escape
