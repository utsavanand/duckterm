...
Status: phases 1 and 2 implemented on branch `control-tower`, 2026-09-26. Clickable prototype
...
- **Entry point.** The topbar's Ask Oracle button becomes **Oracle**. It
  ...
  panes. It is drawn as an opaque layer over the panes, which stay mounted
  and `inert` underneath. The Ask Oracle chat moves into the page's right rail. The docked
...
- **Hidden terminals.** Opening the tower must not resize hidden terminals.
  ...
  exactly this kind of layout switch. Replacing the panes was tried first and
  failed the B5 regression test: the remounted terminal replayed its output
  one column wider. So terminals stay mounted at their size under the layer.
...
| **Tokens · 7 days** | Tokens processed by Claude Code and Codex in the last 7 UTC days, the share read from cache, and output written. Split by agent | `core/tokens.py`. Claude: per-reply `usage`, counted once per (message id, request id), because Claude writes one line per content block and repeats the usage on each; sub-agent transcripts included. Codex: differences between successive `total_token_usage` running totals. Both bucketed by the record's day | First scan 1.7 s; a refresh reads only appended bytes (0.01 s) |
...
2026-09-26, Claude Code processed about 1.07B tokens and Codex about 0.75B.
About 96% of that input was read from cache, and only 5.6M tokens were
written. An earlier draft said 10.9B: it counted repeated Claude lines twice
and summed whole transcripts touched this week, including older history. A single "tokens used" figure would overstate the real work by a
wide margin, so the tile leads with the total and states the cache share
beside it.

**Scope:** every Claude Code and Codex transcript on this Mac, as in the
prototype the owner approved. Narrowing to DuckTerm sessions stays open.
