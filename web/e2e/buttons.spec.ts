...
test("AGENTS.md modal saves the folder's shared instructions", async ({
  ...
  // Structured editor: add an active rule, save — the folder gets the typed
  // source of truth plus a rendered AGENTS.md.
  await page.getByPlaceholder(/New rule/).fill("Be brief.");
  await page.getByRole("button", { name: "Add", exact: true }).click();
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await expect(page.getByText("Rules saved")).toBeVisible();

  const rendered = readFileSync(join(dir, "AGENTS.md"), "utf8");
  expect(rendered).toContain("Be brief.");
  expect(rendered).toContain("generated by RubberTerm");
  const rules = JSON.parse(
    readFileSync(join(dir, ".duckterm-rules.json"), "utf8"),
  ) as { rules: { id: string; status: string }[] };
  expect(rules.rules).toEqual([
    expect.objectContaining({ id: "be-brief", status: "active" }),
  ]);
});
