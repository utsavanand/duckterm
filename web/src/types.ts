export interface DucktermEvent {
  _id: string;
  _ts: number;
  event_type?: string;
  session_key?: string;
...
export interface PersistedSession {
  ...
  launched?: number | null; // 1 if Duckterm launched the session (authoritative)
...
// checkout, but for a Duckterm worktree it's the branch key (…/worktrees/
// <repo>/<branch>) — there, the server's source_app holds the real repo name.
export function repoNameFrom(
  repoPath?: string | null,
  sourceApp?: string | null,
): string | undefined {
  if (repoPath && repoPath.includes("/.duckterm/worktrees/"))
...
export function viewFromPersisted(s: PersistedSession): SessionView {
  return {
    ...
    // Duckterm owns a live PTY only for the in-process launch path. The
...
export function sessionKeyOf(e: DucktermEvent): string | undefined {
