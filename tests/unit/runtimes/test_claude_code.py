...
from duckterm.runtimes.claude_code import ClaudeCodeRuntime, parse_transcript, project_slug


def test_project_slug_dashes_every_non_alphanumeric() -> None:
    # Claude replaces /, ., and _ with dashes — not just /. A worktree under
    # ~/.duckterm gets a DOUBLE dash where `/.` was; the old replace("/","-")
    # kept the dot and looked in a directory that never existed.
    assert (
        project_slug(Path("/Users/sumo/.duckterm/worktrees/omni-cli/test-fork"))
        == "-Users-sumo--duckterm-worktrees-omni-cli-test-fork"
    )
...
def test_locate_transcript_finds_a_dotted_worktree_path(
    ...
    slug = "-Users-sumo--duckterm-worktrees-omni-test-fork"  # double dash for /.
