...
def test_typed_rules_round_trip_and_render(tmp_path: Path) -> None:
    """POST {dir, rules} writes .duckterm-rules.json and renders AGENTS.md;
    GET returns the rules plus the rendered text with managed=true."""
    ...
    async def scenario() -> tuple[dict, str]:
        ...
        async with srv:
            rules = [
                {"id": "no-tabs", "text": "No tabs.", "status": "active", "heading": "Style"},
                {
                    "id": "codex-thing",
                    "text": "Codex-only rule.",
                    "status": "active",
                    "scope": "codex",
                },
                {"id": "pending", "text": "Not yet reviewed.", "status": "candidate"},
            ]
            payload = json.dumps({"dir": str(tmp_path), "rules": rules}).encode()
            ...
            return after, (tmp_path / "AGENTS.md").read_text()

    after, on_disk = asyncio.run(scenario())
    assert after["managed"] is True
    assert [r["id"] for r in after["rules"]] == ["no-tabs", "codex-thing", "pending"]
    assert "generated by RubberTerm" in on_disk
    assert "No tabs." in on_disk
    assert "Codex-only rule. *(codex only)*" in on_disk
    assert "Not yet reviewed." not in on_disk  # candidates don't render
    assert after["text"] == on_disk  # GET returns the rendered file


def test_write_rules_rejects_bad_status(tmp_path: Path) -> None:
    async def scenario() -> int:
        ...
        async with srv:
            payload = json.dumps(
                {"dir": str(tmp_path), "rules": [{"id": "x", "text": "y", "status": "maybe"}]}
            ).encode()
            status, _ = await _request(
                port,
                b"POST /agents-md HTTP/1.1\r\nHost: localhost\r\n"
