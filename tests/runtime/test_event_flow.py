...
from duckterm.server import SELF_PROBE_HEADER, Server
...
def test_tombstoned_session_events_are_all_dropped(tmp_path: Path) -> None:
    ...
    Pulse feed); `duckterm restart` is the way to bring back a live session
    deleted by mistake."""
...
def test_clear_tombstones_revives_deleted_sessions(tmp_path: Path) -> None:
    """duckterm restart clears tombstones so a still-running agent reappears."""
    from duckterm.persistence.history import HistoryStore
...
def test_stop_without_a_pty_reports_not_stopped(tmp_path: Path) -> None:
    """A session running in the user's own terminal (duckterm run / a tab we
    opened) has no PTY supervisor and isn't ours to kill — stop must say so
    rather than mark a still-running session as stopped."""
    from duckterm.persistence.history import HistoryStore
    ...
    def post_expecting_404(port: int) -> dict[str, object]:
        try:
            return dict(json.loads(_post(port, "/sessions/tabbed/stop")))
    ...
    async def scenario() -> dict[str, object]:
        ...
        port = server.sockets[0].getsockname()[1]
        async with server:
            return await asyncio.to_thread(post_expecting_404, port)
    ...
    async def scenario() -> int:
        ...
        async with server:
            req = urllib.request.Request(
                ...
                data=b"",
                headers={"X-Duckterm-Token": _token()},
                method="POST",
            )
            try:
