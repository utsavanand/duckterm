...
def test_snapshot_list_and_restore(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr("duckterm.agents.terminal.platform.system", lambda: "Unknown")
...
def test_restore_resolves_copilot_conversation_id(tmp_path: Path) -> None:
    ...
    Duckterm session_key. For copilot it's read from the recorded session_id."""
...
def test_restore_publishes_sessionstart_so_it_shows_up(
    ...
    # Restore opens a terminal Duckterm owns, so it becomes a launched session
